...
def create_custom(custom: CustomIn, response: Response, repo: CustomRepository = Depends()):
    return repo.create(custom)

@router.get("/customs/{custom_id}", response_model=Optional[CustomOut])
def get_one_custom(custom_id: int, response: Response, repo:CustomRepository = Depends()) -> CustomOut:
